    ...
    <main class="max-w-7xl mx-auto px-6 py-12">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
            ...
            <div class="lg:col-span-1 space-y-8">
                ...
                <div class="space-y-4">
                    <h2 class="text-3xl font-bold">{{ $singleTeacher->name }}</h2>
                    <p class="text-lime-accent font-medium">{{ $singleTeacher->specialization }}</p>
                    <div class="flex space-x-4">
                        <div
                            class="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center hover:bg-lime-accent hover:text-black cursor-pointer transition">
                            in</div>
                        <div
                            class="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center hover:bg-lime-accent hover:text-black cursor-pointer transition">
                            tw</div>
            ...
            <div class="lg:col-span-2 space-y-12">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="bg-[#1A1A1A] border border-gray-800 p-6 rounded-3xl text-center">
                        <div class="text-3xl font-bold lime-accent">{{ $singleTeacher->students_count }}</div>
                        <div class="text-xs text-gray-500 uppercase mt-2">Students</div>
                    </div>
                    <div class="bg-[#1A1A1A] border border-gray-800 p-6 rounded-3xl text-center">
                        <div class="text-3xl font-bold lime-accent">{{ $singleTeacher->rating }}</div>
                        <div class="text-xs text-gray-500 uppercase mt-2">Rating</div>
                    </div>
                    <div class="bg-[#1A1A1A] border border-gray-800 p-6 rounded-3xl text-center">
                        <div class="text-3xl font-bold lime-accent">{{ $singleTeacher->lesson_count }}</div>
                        <div class="text-xs text-gray-500 uppercase mt-2">Courses</div>
                    </div>
                    <div class="bg-[#1A1A1A] border border-gray-800 p-6 rounded-3xl text-center">
                        <div class="text-3xl font-bold lime-accent">{{ $singleTeacher->reviews_count }}</div>
                        <div class="text-xs text-gray-500 uppercase mt-2">Reviews</div>
                    </div>
                </div>

                <div>
                    <h3 class="text-2xl font-bold mb-8">Courses by {{ $singleTeacher->name }}</h3>
                    <div class="space-y-6">

                        @if ($singleTeacher->courses->isNotEmpty())
                            @foreach ($singleTeacher->courses as $item)
                                <div
                                    class="bg-[#1A1A1A] border border-gray-800 p-6 rounded-[2rem] flex flex-col md:flex-row items-center gap-6 hover:border-lime-400/30 transition">
                                    <div class="w-full md:w-48 h-32 bg-gray-800 rounded-2xl overflow-hidden">
                                        <img src="https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&q=80&w=400"
                                            class="w-full h-full object-cover">
                                    </div>
                                    <div class="flex-1">
                                        <h4 class="text-xl font-bold">{{ $item->name }}</h4>
                                        <p class="text-gray-400 text-sm mt-2 line-clamp-1">{{ $item->description }}</p>
                                    ...
                                    <div class="flex gap-3 w-full md:w-auto">
                                        <a href="{{ route('add-to-cart', $item->slug) }}"
                                            class="flex-1 md:flex-none px-6 py-3 bg-lime-accent text-black font-bold rounded-xl text-sm">Add
                                            to Cart</a>
                                        <a href="{{ route('course-detail', $item->slug) }}"
                                            class="flex-1 md:flex-none px-6 py-3 border border-gray-700 rounded-xl text-sm hover:bg-gray-800 transition">View</a>
                                    </div>
                                </div>
                            @endforeach
                        @else
                            <p class="text-gray-400 text-sm mt-3">No courses available for this instructor at the
